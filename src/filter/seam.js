function Seam() {
    ...
    this.energyShader = new Shader(null, '\
           ...
           void main() {\
               ...
               vec2 dy = vec2(0.0, 1.0 / texSize.y);\
               \
               float topLeft = texture2D(texture, texCoord + dx * -1.0 + dy * -1.0).g;\
               float topMiddle = texture2D(texture, texCoord + dy * -1.0).g;\
               float topRight = texture2D(texture, texCoord + dx * 1.0 + dy * -1.0).g;\
               \
               float energy = min(topLeft, min(topMiddle, topRight)) + texture2D(texture, texCoord).g;\
